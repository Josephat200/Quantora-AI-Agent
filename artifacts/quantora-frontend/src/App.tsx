...
function SettingsView({ user, setUser, theme, setTheme, onLogout }: { user: string; setUser: (name: string) => void; theme: 'dark' | 'light'; setTheme: (theme: 'dark' | 'light') => void; onLogout: () => void }) {
  ...
  const [email, setEmail] = useState(`${user.toLowerCase().replace(/\s+/g, '.')}@quantora.app`);
  const [saved, setSaved] = useState(false);
  const [smartContext, setSmartContext] = useState(true);
  const save = (event: FormEvent) => { event.preventDefault(); setUser(name.trim() || user); localStorage.setItem('quantora-email', email); setSaved(true); window.setTimeout(() => setSaved(false), 1800); };
